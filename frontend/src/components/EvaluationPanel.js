    ...
    <section>
        ...
        {METRIC_CARDS.map(m => (
          ...
          >
            <div style={styles.metricHeader}>
              <strong style={{ fontSize: 18, color: "#1e3a8a" }}>
                {m.name}
              </strong>

              <span style={{ fontSize: 22, fontWeight: 800, color: m.color }}>
                {m.score}
              </span>
            </div>
      ...
      <div className="card">
        ...
        <p className="muted" style={{ marginTop: 0, marginBottom: 6 }}>
          NLLB-200 evaluated zero-shot on a 100-pair held-out
          Sesotho–English medical test set using SacreBLEU. The test set was
          kept separate from the corpus lookup cache.
        </p>
            ...
            <tbody>
              {METRICS.map(r => (
                <tr key={r.model}>
                  ...
                  <td style={{ fontWeight: r.bleu !== "—" ? 700 : 400 }}>
                    {r.bleu}
                  </td>

                  <td style={{ fontWeight: r.chrf !== "—" ? 700 : 400 }}>
                    {r.chrf}
                  </td>

                  <td style={{ fontWeight: r.ter !== "—" ? 700 : 400 }}>
                    {r.ter}
                  </td>

                  <td>
                    <StatusPill value={r.status}>{r.label}</StatusPill>
                  </td>
      ...
      <div className="card" style={{ marginTop: 22 }}>
        ...
        <p className="muted" style={{ marginTop: 0, marginBottom: 16 }}>
          ...
          sentence can score well on BLEU while containing a clinically dangerous
          error.
        </p>
        ...
        <div style={styles.errorGrid}>
          {ERROR_ANALYSIS.map(e => (
            ...
            >
              <span style={{ fontSize: 28, fontWeight: 800, color: e.color }}>
                {e.count}
              </span>
        ...
        <Alert type="error" style={{ marginTop: 18 }}>
          <strong>⚠️ Critical Finding — Row 7</strong>
          <p
            style={{
              margin: "6px 0 0",
              fontSize: 13,
      ...
      <div className="card" style={{ marginTop: 22 }}>
        ...
        <p className="muted" style={{ marginTop: 0 }}>
          ...
          Sesotho-English medical translations, but it is not clinically safe as
          a standalone translator. The safest prototype strategy is therefore a
          hybrid approach:
        </p>

        <ul style={styles.list}>
          <li>
            Prefer <strong>verified corpus matches</strong> where available.
          </li>
          <li>
            Use <strong>NLLB-200 generation</strong> only as a fallback.
          </li>
          <li>
            Display <strong>medical disclaimers and high-risk warnings</strong>.
          </li>
          <li>
            Require <strong>bilingual professional review</strong> before any
            clinical use.
          </li>
        </ul>
      </div>
